/*
 * @Author: ye.chen
 * @Date: 2019-11-21 10:06:14
 * @Description: hooks 请求组件
 */
...
export type UseAPIProps<T = any> = RequestProps & RequestInit & OperateProps<T>;
...
globalMethod = request as any;

const useAPI = <Result = any>(opt: UseAPIProps<Result>) => {
  // 组件内部请求方法
  // const _method = opt.request || globalMethod || fetch;
  // 内置 options 对象
  const _options = useMemo<OperateProps<Result>>(() => {
    const {
      manual,
      pollingInterval,
      onSuccess,
  ...
  const fn = useCallback(
    async (attach?: object) => {
      ...
      if (request) {
        res = await request({ ...data, ...payload, ...attach });
      } else if (url) {
        if (globalMethod) {
  ...
  const run = useCallback((...args: any[]): Promise<Result | undefined> => {
    ...
    _options.suspense && setRunner(task);
    return task;
  }, []);
  ...
  return {
    ...
    error: state.error,
    data: (_options.dataFormat as Function)(state.data),
    cancel,
